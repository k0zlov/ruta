...
class RutaBuilder implements Builder {
  ...
  Future<void> build(BuildStep buildStep) async {
    ...
    if (hasEntrypoint) {
      ...
      hasRun = library.topLevelFunctions.any((e) => e.name == 'run');
      hasInit = library.topLevelFunctions.any((e) => e.name == 'init');
    }
    ...
    for (final assetId in assetIds) {
      ...
      for (final element in library.classes) {
        if (element.metadata.annotations.any((meta) {
          ...
          imports.add("'package:${element.library.uri.path}'");
          final constructor =
              element.constructors.firstWhereOrNull(
          ...
          final dependencies = constructor.formalParameters
              .map((param) {
                ...
                final ElementAnnotation? elementAnnotation = param
                    .metadata
                    .annotations
                    .firstWhereOrNull((s) => s.element?.displayName == 'Named');

                if (elementAnnotation == null) return '$result)';

                final constantValue = elementAnnotation.computeConstantValue();

                final nameValue = constantValue
                    ?.getField('name')
                    ?.toStringValue();
          ...
          final endpointGetters = element.getters
              .where(
                (accessor) =>
    ...
    if (!hasEntrypoint) {
      buffer.writeln('// Warning: file lib/main.dart was not found');
    }
    if (!hasInit) {
      buffer
